...
@section('content')
    <!-- CDN FontAwesome untuk Icon Peta -->
    ...
    <header class="relative pt-16 md:pt-24 pb-12 px-5 sm:px-8 lg:px-24 text-center overflow-hidden bg-[#0b0e14]">
        <div
            class="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-red-600/20 blur-[120px] pointer-events-none">
        </div>
        <div class="max-w-6xl mx-auto relative z-10">
            <h1 class="text-4xl sm:text-5xl md:text-6xl font-black tracking-tighter mb-4 uppercase leading-[1.1]">
                Jelajah Wisata & <br class="block sm:hidden">
                <span class="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-rose-400">Tongkrongan</span>
            </h1>
            <p class="text-gray-400 max-w-xl mx-auto text-xs md:text-sm font-medium leading-relaxed">
                Daftar lengkap destinasi pilihan dan tempat nongkrong asyik di Kota Mataram.
            </p>
        </div>
    </header>
    ...
    <main class="max-w-7xl mx-auto px-5 sm:px-8 lg:px-24 py-6 md:py-10 w-full min-h-screen relative z-10">
        ...
        <div class="mb-8 md:mb-10 w-full flex justify-start">
            <a href="{{ url('/#wisata-tongkrongan') }}" wire:navigate
                ...
                <svg class="w-3.5 h-3.5 mr-2.5 group-hover:-translate-x-1 transition-transform" fill="none"
        ...
        <div class="flex items-center gap-3 mb-10 md:mb-12 w-full overflow-x-auto no-scrollbar snap-x snap-mandatory pb-4">
            ...
            <button onclick="filterKategori('all')" id="btn-all"
                ...
                <i class="fa-solid fa-map-location-dot mr-2"></i>Semua Kategori
            </button>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 w-full" id="grid-data">
            @forelse($semuaData as $item)
                ...
                {{-- Perubahan: Menggunakan <a> dengan wire:navigate agar tidak reload --}}
                    <a href="{{ route('wisata.show', $item->id) }}" wire:navigate
                        ...
                        <div class="relative h-48 md:h-52 w-full shrink-0 p-2.5 md:p-3">
                            ...
                            <span
                                class="absolute top-5 left-5 md:top-6 md:left-6 bg-red-600 text-white text-[8px] md:text-[9px] font-black px-3.5 py-1.5 rounded-full uppercase tracking-widest shadow-md">
                                {{ $item->kategori }}
                            </span>
                        </div>
                        ...
                        <div class="p-5 md:p-6 pt-3 flex flex-col flex-grow">
                            <h3
                                class="text-lg md:text-xl font-black text-slate-900 line-clamp-1 mb-2.5 uppercase group-hover:text-red-600 transition-colors leading-tight">
                                {{ $item->nama_wisata }}
                            </h3>

                            <p
                                class="text-slate-600 text-[11px] md:text-xs leading-relaxed line-clamp-2 md:line-clamp-3 mb-5 font-medium">
                            ...
                            <div class="flex items-center justify-between pt-4 border-t border-slate-100 mt-auto">
                                @if($item->maps_url)
                                    <div
                                        class="text-slate-500 text-[9px] md:text-[10px] font-bold flex items-center gap-1.5 uppercase tracking-widest">
                                        <i class="fa-solid fa-map-location-dot text-red-500"></i> Ada Peta
                                    </div>
                                @else
                                    <div
                                        class="text-slate-400 text-[9px] font-bold flex items-center gap-1.5 uppercase tracking-widest italic">
                                        Peta Belum Ada
                                    </div>
                                @endif

                                <div
                                    class="inline-flex items-center gap-1.5 text-red-600 text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-colors">
                                    Detail <i
                                        class="fa-solid fa-arrow-right transform group-hover:translate-x-1.5 transition-transform duration-300"></i>
                                </div>
                            </div>
                        </div>
                    </a>
            @empty
                    <div
                        class="col-span-full text-center py-20 bg-[#161b22] rounded-[30px] border-2 border-dashed border-white/10 shadow-xl">
                        <div class="text-gray-600 mb-4"><i class="fa-solid fa-map-location text-4xl"></i></div>
                        <p class="text-gray-500 font-bold uppercase tracking-[0.3em] text-[10px] md:text-xs">Belum ada data
                            destinasi.</p>
                    </div>
                @endforelse
        </div>
    ...
    <footer class="bg-black py-12 px-6 border-t border-white/5 text-center mt-10">
        ...
        <p
            ...
            &copy; 2026 Radio Kota Mataram. All Rights Reserved.</p>
    </footer>
